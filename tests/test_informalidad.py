...
def _ei(df: pd.DataFrame, anio: int = 2025) -> int | None:
    ...
    """
    informal = clasificar_informalidad_dane(df, anio_referencia=anio,
                                            verbose=False)
    val = informal.iloc[0]
    if pd.isna(val):
        return None
...
class TestGobierno:
    ...
    def test_gobierno_con_cotizaciones_es_formal(self):
        df = _row_to_df(P6430=2, P6100=1, P6110=4,
                        P6920=1, P6930=1, P6940=3)
        assert _ei(df) == 1
...
class TestIndependientesSinNegocio:
    ...
    def test_cta_propia_sin_neg_contab_NS_directivo(self):
        ...
        df = _row_to_df(P6430=4, P6765=1, P3065=9, P3066=9,
                        OFICIO_C8="1010")
        assert _ei(df) == 1
...
class TestIndependientesConNegocioSinRegistro:
    ...
    def test_cta_propia_p6775_9_directivo(self):
        df = _row_to_df(P6430=4, P6765=7, P3067=2, P6775=9,
                        OFICIO_C8="1510")
        assert _ei(df) == 1
...
class TestAntiRegresionGobierno:
    def test_gobierno_no_es_atajo_explicito(self):
        ...
        assert (info == 0).sum() == n, \
            "Gobierno debe ser 100% formal según SAS oficial"
...
class TestCanarioBoletinDANE:
    ...
    def df_dic(self):
        ruta = os.environ.get(
            "GEIH_PARQUET",
            "/content/drive/MyDrive/ProColombia/GEIH/"
            "GEIH_consolidado_2025.parquet"
        )
    ...
    def test_centros_poblados_rural(self, df_dic):
        ocu_dane, inf_dane, tasa_dane = \
            REF_DANE_DIC_2025["Centros poblados y rural disperso"]
